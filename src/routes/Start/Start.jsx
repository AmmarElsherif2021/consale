...
const Start = () => {
  ...
  //handle langs
  const handleToggle = () => {
    setLang((prev) => (prev === "ar" ? "eng" : "ar"));
  };
  useEffect(() => console.log("lang changed"), [lang]);

  const routesArr = [
    ["Dashboard", " الحسابات", "Countings"],
    ["Stock", "إدارة المخزن", "Stock Management"],
    ["Addbill", "اضف فاتورة", "Add Bill"],
  ];
        ...
        <div className={user.isLogged ? "sidebar-header" : "start-header"}>
          ...
          {user.isLogged ? (
            ...
            <form onSubmit={handleClick} className="start">
              ...
              <button className="lang-btn" onClick={handleToggle}>
                <small>{lang === "eng" ? "عربي" : "English"}</small>
              </button>
            </form>
          )}
